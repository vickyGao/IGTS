...
public class HotResourceTest extends TestBase {

    private static final String BASE_PATH = "hot/";
    ...
    public void testCreate() {
        ...
        Hot testHot = new Hot();
        testHot.setCommodityId(commodity.getId());
        testHot.setDescription("for test");
        testHot.setDisplaySequence(0);

        Response createHotResponse = doPost(BASE_PATH + "entity", adminToken, JsonUtil.getJsonStringFromPojo(testHot));
        assertEquals("Create hot commodity failed", Status.OK.getStatusCode(), createHotResponse.getStatus());

        hot = CommonUtil.getEntityFromResponse(createHotResponse, Hot.class);
    }
    ...
    public void testUpdate() {
        ...
        Response updateHotResponse = doPut(BASE_PATH + "entity", adminToken, JsonUtil.getJsonStringFromPojo(hot));
        assertEquals("Update hot commodity failed", Status.OK.getStatusCode(), updateHotResponse.getStatus());
    }

    @Test
    @Order(20)
    public void testGetSliceById() {
        String path = BASE_PATH + "entity/" + hot.getId();
        Response response = doGet(path, adminToken);
        assertEquals("Get hot commodity by id failed", Status.OK.getStatusCode(), response.getStatus());
    }
    ...
    public void testGetHotCommodities() {
        ...
        assertEquals("Get hot commodities failed", Status.OK.getStatusCode(), response.getStatus());
    }
    ...
    public void testDelete() {
        ...
        assertEquals("Delete hot commodities failed", Status.OK.getStatusCode(), response.getStatus());
    }
